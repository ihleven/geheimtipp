<template>
  <main>
    <section class="container bg-white">
      ...
      <div class="[grid-area:heim] bg-white text-gray-700 text-xl font-semibold text-right">
        {{ match.teams[0] }}
      </div>
      <div class="[grid-area:result] text-center">{{ match.result }}</div>
      <div class="[grid-area:gast] bg-white text-gray-700 text-xl font-semibold">
        {{ match.teams[1] }}
      </div>
    ...
    <section>
      <ul class="bg-white border-y border-gray-400 divide-y divide-gray-300">
        <li v-for="tipp in match.odds.tipps" :key="tipp.login" class="flex items-center justify-between">
          {{ tipp.login }} {{ tipp.ht }}:{{ tipp.at }} - {{ tipp.Punkte }}Pkt.

          <button class="flex items-center pl-2 py-2 text-sm text-blue-500 hover:text-white bg-black/30">
            zum Spiel
...
const { data } = await useFetch(`/api/matches/${id}`);
